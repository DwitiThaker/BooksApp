...
async def create_book(book_request: BookRequest = BaseModel()):
...
@app.delete("/books/{book_id}", status_code=status.HTTP_204_NO_CONTENT)  #Path parameter version
async def delete_book(book_id: int = Path(gt=0)):  #extra validation to path parameter
    book_changed = False
    for i in range(len(BOOKS)):
...
async def publish_date(published_date: int = Query(lt=2031, gt=1999)):
    books_to_return = []
    for book in BOOKS:
...
@app.get("/books/", status_code=status.HTTP_200_OK)     #Query parameter version 
async def find_book_by_rating(book_rating: int = Query(get = 0, lt = 6)): 
    books_to_return = []
    for book in BOOKS:
